...
namespace OrderMs.Application.Commands
{
    ...
    public class CancelOrderCommandHandler : IRequestHandler<CancelOrderCommand, Guid>
    {
        ...
        public CancelOrderCommandHandler(IOrderRepository orderRepository, IIncidentRepository incidentRepository, IPolicyRepository policyRepository, IAdditionalCostRepository additionalCostRepository, IFeeRepository feeRepository, IProviderService providerService, IGoogleApiService googleApiService)
        {
        ...
        public async Task<Guid> Handle(CancelOrderCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (order.State == OrderState.Canceled) throw new InvalidAttributeException("Order already Canceled");
                if (order.State == OrderState.Finished) throw new InvalidAttributeException("Order is Finished");
                ...
                decimal totalDistance = (decimal)((originToIncident.DistanceValue) * 0.001);
                ...
                order = Order.Update(order, null, costoFinal, null, OrderState.Canceled, null, null);
